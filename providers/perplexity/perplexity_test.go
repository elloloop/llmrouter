...
func TestNew(t *testing.T) {
	t.Run("succeeds with api key", func(t *testing.T) {
		p, err := perplexity.New(llmrouter.WithAPIKey("test-key"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		if p == nil {
	...
	t.Run("returns error when api key empty", func(t *testing.T) {
		_, err := perplexity.New(llmrouter.WithAPIKey(""))
		if err == nil {
			t.Fatal("expected error for empty api key")
		}
	})

	t.Run("default base URL is perplexity", func(t *testing.T) {
		if perplexity.DefaultBaseURL != "https://api.perplexity.ai" {
			t.Errorf("DefaultBaseURL = %q, want https://api.perplexity.ai", perplexity.DefaultBaseURL)
		}
	})

	t.Run("user supplied WithBaseURL overrides default", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{"content":"ok"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()

		p, err := perplexity.New(
	...
	t.Run("nil option is tolerated", func(t *testing.T) {
		_, err := perplexity.New(llmrouter.WithAPIKey("test-key"), nil)
		if err != nil {
...
func TestName(t *testing.T) {
	t.Run("returns perplexity", func(t *testing.T) {
		p, err := perplexity.New(llmrouter.WithAPIKey("test-key"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		if got := p.Name(); got != "perplexity" {
			t.Errorf("Name = %q, want perplexity", got)
		}
	})
...
func TestCompletionStream(t *testing.T) {
	t.Run("forwards chunks end to end", func(t *testing.T) {
		payloads := []string{
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"}}]}`,
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{"content":" world"}}]}`,
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}`,
		}
	...
	t.Run("preserves raw wire bytes", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{"content":"r"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()
	...
	t.Run("rewrites 401 ErrUpstream to perplexity", func(t *testing.T) {
		...
		if ue.Provider != "perplexity" {
			t.Errorf("Provider = %q, want perplexity", ue.Provider)
		}
	...
	t.Run("rewrites 429 ErrUpstream to perplexity", func(t *testing.T) {
	...
	t.Run("rewrites 500 ErrUpstream to perplexity", func(t *testing.T) {
	...
	t.Run("error message contains provider name", func(t *testing.T) {
		...
		if err == nil || !strings.Contains(err.Error(), "perplexity") {
			t.Errorf("err = %v, want substring 'perplexity'", err)
		}
	})
	...
	t.Run("handles single chunk stream", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"p1","object":"chat.completion.chunk","created":1,"model":"sonar","choices":[{"index":0,"delta":{"content":"only"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()
		p, err := perplexity.New(
			llmrouter.WithAPIKey("test-key"),
