...
func TestDefaultBaseURL_PointsAtDeepSeek(t *testing.T) {
...
func TestNew_NilOptionsAreIgnored(t *testing.T) {
	if _, err := deepseek.New(llmrouter.WithAPIKey("test-key"), nil); err != nil {
		t.Fatalf("New with nil option: %v", err)
	}
}

func TestName_ReturnsDeepSeek(t *testing.T) {
	p, err := deepseek.New(llmrouter.WithAPIKey("test-key"))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestCompletionStream_ForwardsChunksAndCompletes(t *testing.T) {
	payloads := []string{
		`{"id":"chatcmpl-1","object":"chat.completion.chunk","created":1,"model":"deepseek-chat","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"}}]}`,
		`{"id":"chatcmpl-1","object":"chat.completion.chunk","created":1,"model":"deepseek-chat","choices":[{"index":0,"delta":{"content":" world"}}]}`,
		`{"id":"chatcmpl-1","object":"chat.completion.chunk","created":1,"model":"deepseek-chat","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}`,
	}
...
func TestCompletionStream_UpstreamError_RewritesProviderToDeepSeek(t *testing.T) {
	...
	if upErr.Provider != "deepseek" {
		t.Errorf("Provider = %q, want deepseek", upErr.Provider)
	}
...
func TestCompletionStream_PreservesRawWireBytes(t *testing.T) {
	payload := `{"id":"raw-1","object":"chat.completion.chunk","created":1,"model":"deepseek-chat","choices":[{"index":0,"delta":{"content":"x"}}]}`
...
func TestCompletionStream_DefaultBaseURL_AppliedWhenAbsent(t *testing.T) {
	// We cannot reach the real DeepSeek host from tests, but we can verify
	// that without WithBaseURL the inner provider attempts a request
	// targeting the DeepSeek host (the stub transport surfaces the URL
	// in its error so we can assert against it).
	p, err := deepseek.New(
		llmrouter.WithAPIKey("test-key"),
	...
	if !strings.Contains(err.Error(), "https://api.deepseek.com/v1/chat/completions") {
		t.Errorf("error = %v, want URL containing DeepSeek default", err)
	}
}
...
func TestCompletionStream_ErrorMessageMentionsDeepSeek(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "deepseek") {
		t.Fatalf("error = %v, want substring 'deepseek'", err)
	}
