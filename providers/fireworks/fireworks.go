// Package fireworks is a thin wrapper around the openai provider that
// targets Fireworks AI's OpenAI-compatible Chat Completions API.
//
// Fireworks AI exposes a fully OpenAI-compatible endpoint at
// https://api.fireworks.ai/inference/v1, so this package delegates all
// transport, SSE parsing, and chunk normalization to providers/openai.
// The only behavioural differences are:
//
//   - Name() returns "fireworks".
//   - The default base URL points at Fireworks AI.
//   - *llmrouter.ErrUpstream values surfaced by the inner provider have
//     their Provider field rewritten from "openai" to "fireworks" so
//     downstream code, logs, and metrics can attribute the failure
//     correctly.
//
// Quick start:
//
//	p, err := fireworks.New(llmrouter.WithAPIKey(os.Getenv("FIREWORKS_API_KEY")))
package fireworks
...
// DefaultBaseURL is Fireworks AI's OpenAI-compatible Chat Completions
// endpoint.
...
// Provider implements llmrouter.Provider against Fireworks AI by
// delegating to a configured openai.Provider.
type Provider struct {
	inner *openai.Provider
}

// New constructs a Fireworks AI provider. The default base URL targets
// Fireworks AI; pass llmrouter.WithBaseURL to override (e.g. for a
// self-hosted proxy).
...
// any *llmrouter.ErrUpstream so its Provider field reads "fireworks".
