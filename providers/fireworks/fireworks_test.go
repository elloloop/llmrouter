...
func TestFireworksProvider(t *testing.T) {
	...
	t.Run("NameReturnsFireworks", func(t *testing.T) {
		p, err := fireworks.New(llmrouter.WithAPIKey("k"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		if got := p.Name(); got != "fireworks" {
			t.Errorf("Name = %q, want fireworks", got)
		}
	})
	...
	t.Run("NewWithWhitespaceAPIKeyFails", func(t *testing.T) {
		_, err := fireworks.New(llmrouter.WithAPIKey("   "))
		if err == nil {
	...
	t.Run("NewIgnoresNilOption", func(t *testing.T) {
		_, err := fireworks.New(nil, llmrouter.WithAPIKey("k"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
	})
	...
	t.Run("NewWithEmptyBaseURLFails", func(t *testing.T) {
		_, err := fireworks.New(llmrouter.WithAPIKey("k"), llmrouter.WithBaseURL(""))
		if err == nil {
	...
	t.Run("CompletionStreamForwardsContent", func(t *testing.T) {
		payloads := []string{
			...
			`{"id":"a","object":"chat.completion.chunk","created":1,"model":"m","choices":[{"index":0,"delta":{"content":" fireworks"}}]}`,
		}
		srv, _ := fakeSSEServer(t, payloads)
		defer srv.Close()

		p, err := fireworks.New(llmrouter.WithAPIKey("k"), llmrouter.WithBaseURL(srv.URL))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		...
		if content != "Hello fireworks" {
			t.Errorf("content = %q, want %q", content, "Hello fireworks")
		}
	})
	...
	t.Run("CompletionStreamPreservesRawBytes", func(t *testing.T) {
		...
		p, _ := fireworks.New(llmrouter.WithAPIKey("k"), llmrouter.WithBaseURL(srv.URL))
	...
	t.Run("UpstreamErrorMessageIncludesFireworks", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		p, _ := fireworks.New(llmrouter.WithAPIKey("k"), llmrouter.WithBaseURL(srv.URL))
		_, err := p.CompletionStream(context.Background(), sampleChatRequest())
		if err == nil || !strings.Contains(err.Error(), "fireworks upstream") {
			t.Errorf("err = %v, want to contain 'fireworks upstream'", err)
		}
	})
	...
	t.Run("AuthorizationHeaderSent", func(t *testing.T) {
		...
		p, _ := fireworks.New(llmrouter.WithAPIKey("secret-key"), llmrouter.WithBaseURL(srv.URL))
		stream, err := p.CompletionStream(context.Background(), sampleChatRequest())
		if err != nil {
			t.Fatalf("CompletionStream: %v", err)
		}
	...
	t.Run("NonUpstreamErrorPassThrough", func(t *testing.T) {
		...
		p, _ := fireworks.New(llmrouter.WithAPIKey("k"), llmrouter.WithBaseURL(closedURL))
		_, err := p.CompletionStream(context.Background(), sampleChatRequest())
		if err == nil {
	...
	t.Run("ProviderImplementsLLMRouterProvider", func(t *testing.T) {
		var _ llmrouter.Provider = (*fireworks.Provider)(nil)
	})
}
