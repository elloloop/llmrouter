// Package groq is a thin wrapper around the openai provider that targets
// Groq's OpenAI-compatible Chat Completions API.
//
// Groq exposes a fully OpenAI-compatible endpoint (including
// stream_options.include_usage) at https://api.groq.com/openai/v1, so this
// package delegates all transport, SSE parsing, and chunk normalization to
// providers/openai. The only behavioural differences are:
//
//   - Name() returns "groq".
//   - The default base URL points at Groq.
//   - *llmrouter.ErrUpstream values surfaced by the inner provider have
//     their Provider field rewritten from "openai" to "groq" so downstream
//     code, logs, and metrics can attribute the failure correctly.
//
// Quick start:
//
//	p, err := groq.New(llmrouter.WithAPIKey(os.Getenv("GROQ_API_KEY")))
package groq
...
// DefaultBaseURL is Groq's OpenAI-compatible Chat Completions endpoint.
...
// Provider implements llmrouter.Provider against Groq by delegating to a
// configured openai.Provider.
type Provider struct {
	inner *openai.Provider
}

// New constructs a Groq provider. The default base URL targets Groq; pass
...
// any *llmrouter.ErrUpstream so its Provider field reads "groq".
