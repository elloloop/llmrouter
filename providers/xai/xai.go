// Package xai is a thin wrapper around the openai provider that targets
// xAI's OpenAI-compatible Chat Completions API (Grok).
//
// xAI exposes a fully OpenAI-compatible endpoint at https://api.x.ai/v1,
// so this package delegates all transport, SSE parsing, and chunk
// normalization to providers/openai. The only behavioural differences
// are:
//
//   - Name() returns "xai".
//   - The default base URL points at xAI.
//   - *llmrouter.ErrUpstream values surfaced by the inner provider have
//     their Provider field rewritten from "openai" to "xai" so downstream
//     code, logs, and metrics can attribute the failure correctly.
//
// Quick start:
//
//	p, err := xai.New(llmrouter.WithAPIKey(os.Getenv("XAI_API_KEY")))
package xai
...
// DefaultBaseURL is xAI's OpenAI-compatible Chat Completions endpoint.
...
// Provider implements llmrouter.Provider against xAI by delegating to a
// configured openai.Provider.
type Provider struct {
	inner *openai.Provider
}

// New constructs an xAI provider. The default base URL targets xAI; pass
// llmrouter.WithBaseURL to override (e.g. for a self-hosted proxy).
...
// any *llmrouter.ErrUpstream so its Provider field reads "xai".
