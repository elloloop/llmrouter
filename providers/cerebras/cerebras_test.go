...
func TestNew(t *testing.T) {
	t.Run("succeeds with api key", func(t *testing.T) {
		p, err := cerebras.New(llmrouter.WithAPIKey("test-key"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		if p == nil {
	...
	t.Run("returns error when api key empty", func(t *testing.T) {
		_, err := cerebras.New(llmrouter.WithAPIKey(""))
		if err == nil {
			t.Fatal("expected error for empty api key")
		}
	})

	t.Run("default base URL is cerebras", func(t *testing.T) {
		if cerebras.DefaultBaseURL != "https://api.cerebras.ai/v1" {
			t.Errorf("DefaultBaseURL = %q, want https://api.cerebras.ai/v1", cerebras.DefaultBaseURL)
		}
	})

	t.Run("user supplied WithBaseURL overrides default", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{"content":"ok"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()
	...
	t.Run("nil option is tolerated", func(t *testing.T) {
		_, err := cerebras.New(llmrouter.WithAPIKey("test-key"), nil)
		if err != nil {
...
func TestName(t *testing.T) {
	t.Run("returns cerebras", func(t *testing.T) {
		p, err := cerebras.New(llmrouter.WithAPIKey("test-key"))
		if err != nil {
			t.Fatalf("New: %v", err)
		}
		if got := p.Name(); got != "cerebras" {
...
func TestCompletionStream(t *testing.T) {
	t.Run("forwards chunks end to end", func(t *testing.T) {
		payloads := []string{
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{"role":"assistant","content":"Hello"}}]}`,
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{"content":" world"}}]}`,
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}`,
		}
	...
	t.Run("preserves raw wire bytes", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{"content":"r"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()
	...
	t.Run("rewrites 401 ErrUpstream to cerebras", func(t *testing.T) {
		...
		if ue.Provider != "cerebras" {
			t.Errorf("Provider = %q, want cerebras", ue.Provider)
		}
	...
	t.Run("rewrites 429 ErrUpstream to cerebras", func(t *testing.T) {
	...
	t.Run("rewrites 500 ErrUpstream to cerebras", func(t *testing.T) {
	...
	t.Run("error message contains provider name", func(t *testing.T) {
		...
		if err == nil || !strings.Contains(err.Error(), "cerebras") {
			t.Errorf("err = %v, want substring 'cerebras'", err)
		}
	})
	...
	t.Run("handles single chunk stream", func(t *testing.T) {
		srv := fakeSSEServer(t, []string{
			`{"id":"c1","object":"chat.completion.chunk","created":1,"model":"llama3.1-70b","choices":[{"index":0,"delta":{"content":"only"},"finish_reason":"stop"}]}`,
		})
		defer srv.Close()
		p, err := cerebras.New(
			llmrouter.WithAPIKey("test-key"),
