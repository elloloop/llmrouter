// Package cerebras is a thin wrapper around the openai provider that
// targets Cerebras's OpenAI-compatible Chat Completions API.
//
// Cerebras exposes a fully OpenAI-compatible endpoint at
// https://api.cerebras.ai/v1, so this package delegates all transport,
// SSE parsing, and chunk normalization to providers/openai. The only
// behavioural differences are:
//
//   - Name() returns "cerebras".
//   - The default base URL points at Cerebras.
//   - *llmrouter.ErrUpstream values surfaced by the inner provider have
//     their Provider field rewritten from "openai" to "cerebras" so
//     downstream code, logs, and metrics can attribute the failure
//     correctly.
//
// Quick start:
//
//	p, err := cerebras.New(llmrouter.WithAPIKey(os.Getenv("CEREBRAS_API_KEY")))
package cerebras
...
// DefaultBaseURL is Cerebras's OpenAI-compatible Chat Completions
// endpoint.
...
// Provider implements llmrouter.Provider against Cerebras by delegating
// to a configured openai.Provider.
type Provider struct {
	inner *openai.Provider
}

// New constructs a Cerebras provider. The default base URL targets
// Cerebras; pass llmrouter.WithBaseURL to override (e.g. for a
// self-hosted proxy).
...
// any *llmrouter.ErrUpstream so its Provider field reads "cerebras".
