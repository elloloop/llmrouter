// translation.go — Anthropic /v1/messages body assembly + SSE
// translation. Duplicated from providers/azureanthropic/translation.go
// (which is duplicated from providers/anthropic/anthropic.go). The ONLY
// behaviour difference here is buildBody injecting an
// "anthropic_version" key into the outgoing body — Vertex requires it
// in-band rather than in an HTTP header.
//
// When this file diverges from the source-of-truth files, sync changes
// back. A future refactor may extract the shared logic into
// providers/internal/anthropicwire and import it from all three.
...
// /v1/messages JSON body and injects the Vertex-required
// "anthropic_version" field. Mirrors
// providers/azureanthropic.buildBody.
func buildBody(req llmrouter.ChatRequest) ([]byte, error) {
	...
	out := map[string]any{
		"anthropic_version": vertexAnthropicVer,
		"model":             req.Model,
...
func pump(ctx context.Context, resp *http.Response, model string, hooks llmrouter.ProducerHooks) {
	...
	if err := scanner.Err(); err != nil && err != io.EOF {
		hooks.Finish(fmt.Errorf("vertexanthropic: read stream: %w", err))
		return
	}
	hooks.Finish(nil)
}
