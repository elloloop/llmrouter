...
export function getEditUrl(pageUrlPath: string): string {
  const astroPath = pageUrlToAstroPath(pageUrlPath);
  if (!astroPath) {
    return "https://github.com/elloloop/llmrouter/tree/main/docs-site";
  }
  const rel = path.relative(REPO_ROOT, astroPath);
  return `https://github.com/elloloop/llmrouter/edit/main/${rel}`;
}

function pageUrlToAstroPath(pageUrlPath: string): string | null {
  // pageUrlPath looks like "/llmrouter/", "/llmrouter/docs/", or "/llmrouter/docs/quickstart/".
  const base = "/llmrouter";
...
export async function getGithubStars(): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch(
      "https://api.github.com/repos/elloloop/llmrouter",
      {
